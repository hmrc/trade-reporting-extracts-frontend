...
import utils.DateTimeFormats.{dateTimeFormat, formattedTime}
...
class RequestConfirmationController @Inject() (
  ...
  private def getDateAndTime(implicit messages: Messages): (String, String) = {
    (LocalDate.now(clock.withZone(ZoneOffset.UTC)).format(dateTimeFormat()(messages.lang)),
     formattedTime(clock)(messages.lang))
  }
