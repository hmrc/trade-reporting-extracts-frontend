...
import models.thirdparty.EditThirdPartyRequest
...
class EditThirdPartySubmissionHandler @Inject(
  ...
  def submit(thirdPartyEori: String): Action[AnyContent] =
    (identify andThen getData andThen requireData).async { implicit request =>
      val updatedDetails = EditThirdPartyRequest(
        userEORI = request.eori,
        thirdPartyEORI = thirdPartyEori,
        accessStart = request.userAnswers.get(EditThirdPartyAccessStartDatePage(thirdPartyEori))
          .map(_.atStartOfDay().toInstant(ZoneOffset.UTC)),
        accessEnd = request.userAnswers.get(EditThirdPartyAccessEndDatePage(thirdPartyEori))
          .map(_.atStartOfDay().toInstant(ZoneOffset.UTC)),
        //NEED PAGES
        reportDateStart = None,
        reportDateEnd = None,
        accessType = request.userAnswers.get(EditThirdPartyDataTypesPage(thirdPartyEori))
          .map(_.map(_.toString.toUpperCase())),
        referenceName = request.userAnswers.get(EditThirdPartyReferencePage(thirdPartyEori))
      )
