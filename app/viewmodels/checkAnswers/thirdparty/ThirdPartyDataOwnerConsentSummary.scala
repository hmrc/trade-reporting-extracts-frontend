...
object ThirdPartyDataOwnerConsentSummary {

  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(ThirdPartyDataOwnerConsentPage).map { answer =>

      val value = if (answer) "thirdPartyDataOwnerConsent.yes" else "site.no"
      ...
      SummaryListRowViewModel(
        ...
        actions = Seq(
          ActionItemViewModel(
            ...
            .withVisuallyHiddenText(messages("thirdPartyDataOwnerConsent.change.hidden"))
        )
      )
    }
}
