...
class ReportNameSummarySpec extends SpecBase {

  implicit private val messages: Messages = stubMessages()

  "ReportNameSummary.row" - {

    "must return a SummaryListRow when an answer is present" in {
      val answer  = "Test Report Name"
      val answers = UserAnswers("id").set(ReportNamePage, answer).success.value
          ...
          actions = Some(
            Actions(items =
              Seq(
                ActionItemViewModel(
                  "site.change",
                  controllers.report.routes.ReportNameController.onPageLoad(CheckMode).url
