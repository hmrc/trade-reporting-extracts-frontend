...
import forms.EmailSelectionFormProvider
import models.{NormalMode, EmailSelection, UserAnswers}
...
import views.html.EmailSelectionView
...
class EmailSelectionControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val emailSelectionRoute = routes.EmailSelectionController.onPageLoad(NormalMode).url
  ...
  "EmailSelection Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(EmailSelection.values.toSet), NormalMode)(request, messages(application)).toString
      }
    }
