...
class RequestedReportsControllerSpec extends SpecBase with MockitoSugar {
  ...
  "RequestedReports Controller" - {
    ...
    "must display correct status tag and redirect for ERROR" in {
      val mockTradeReportingExtractsService = mock[TradeReportingExtractsService]
      val userReport                        = RequestedUserReportViewModel(
        reportName = "reportName",
        referenceNumber = "referenceNumber",
      ...
      running(application) {
        ...
        val statusCell = document.select("span.govuk-tag--red").first()
    ...
    "must display correct status tag and redirect for NO_DATA_AVAILABLE" in {
      val mockTradeReportingExtractsService = mock[TradeReportingExtractsService]
      val userReport                        = RequestedUserReportViewModel(
        reportName = "reportName",
        referenceNumber = "referenceNumber",
